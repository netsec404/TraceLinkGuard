...
def short_org_name(org: Optional[str]) -> str:
    ...
    if not org:
        return "Unavailable"
    # Normalize common long org strings
    org = org.strip()
...
def get_whois_info(host: str) -> dict:
    try:
        ...
        age_days = (datetime.now() - creation_date).days if creation_date else "Unavailable"
...
def classify_risk(
    ...
    # High-risk TLD hints (adjustable)
    high_risk_tlds = {"biz", "top", "xyz", "country", "work", "click", "link"}
    if tld in high_risk_tlds:
        score += 3; reasons.append(f"High-risk TLD: .{tld}")

    low_org = (org or "").lower()
    vpn_markers = ("vpn", "proxy", "anonymizer")
    dc_markers = ("hosting", "data center", "colo", "llc")
    if any(k in low_org for k in vpn_markers):
        score += 3; reasons.append("Possible VPN/Proxy sender.")
    if any(k in low_org for k in dc_markers):
        score += 2; reasons.append("Likely data center/hosting provider.")

    hops = len(redirects or [])
    if hops >= 4:
        score += 2; reasons.append(f"Long redirect chain: {hops} hops.")
    if hops >= 1:
        try:
            first = urlparse(redirects[0]["url"]).netloc.split(":")[0]
            last = urlparse(redirects[-1]["url"]).netloc.split(":")[0]
            if first and last and (first != last):
                score += 2; reasons.append("Cross-domain redirect chain.")
        except Exception:
            pass
    ...
    if content_type and any(content_type.lower().startswith(rt) for rt in risky_types):
        score += 3; reasons.append(f"Suspicious content type: {content_type}")

    if final_scheme == "http":
        score += 2; reasons.append("No HTTPS on final URL.")

    # Trusted example simplification
...
async def analyze(url: str = Query(..., description="URL to analyze fully")):
    ...
    return {
        ...
        "anonymization_flags": {
            "is_hosting_provider": any(k in (ip_geo.get("org") or "") for k in ("Google", "Amazon", "Microsoft", "Cloudflare")),
            "is_vpn_or_proxy": ("vpn" in (ip_geo.get("org") or "").lower() or "proxy" in (ip_geo.get("org") or "").lower())
        },
        "ip_reputation": None,
        "redirects": redirects,
        "traceroute": hops,
